...
mod tests {
    ...
    fn scan_finds_image_after_zero_region() {
        // The magic is `MZ` followed by 14 zero bytes, and the DOS header stays zero until
        // `e_lfanew`. Aho-Corasick reports matches in order of where they end, so the all-zero
        // program_store magic matches inside those zeros before this image's magic is reported.
